---
title: "WNV L1 phylogeny in Italy and the Mediterranean Basin"
...
colfunc <- colorRampPalette(c("#FEE69A", "#E34A33"))

# Plot the whole area (Italy)
ggplot(data = world) +
    geom_sf(fill="white", color="transparent") +
    geom_sf(data = ita_borders, fill = "white", color="black", size=0.3) +
    geom_point(data = wnv1IZS_data_year, aes(x = longitude, y = latitude, shape=Host_category, color=as.numeric(year)), size = 2, fill = "darkred") +
    coord_sf(xlim = c(6.3, 18.7), ylim = c(36.5, 47.2), expand = FALSE)+
    scale_color_gradient(low="#FEE69A", high="#E34A33")+
...
labs(color="Year", shape = "Host")

# Zoom on northern Italian area
ggplot(data = world) +
    geom_sf(fill="white", color="transparent") +
    geom_sf(data = ita_borders, fill = "white", color="black", size=0.4) +
    geom_point(data = wnv1IZS_data_year, aes(x = longitude, y = latitude, shape=Host_category, color=as.numeric(year)), size = 4) +
...
wnvtree_bay <- read.beast("wnv_L1_mediterranean_clock_subset_trimmed.tre")
...
wnvtree_bay_df_ann <- wnvtree_bay_df_annp1 %>% mutate(posteriorsupport=ifelse(posterior > 0.5 & posterior < 0.9, "medium", ifelse(posterior >= 0.9, "high","low")))
...
wnv_tree_plot <- ggtree(wnvtree_bay, size=1.1,  mrsd="2022-10-03")+
  ...
  geom_nodepoint(aes(color=wnvtree_bay_df_ann$posteriorsupport), size=2.2)+
...
id_host_tab <- read.table(file="id_host_tab_wnv_26.txt", sep="\t", stringsAsFactor=F)
annotated_tree_host <- gheatmap(wnv_tree_plot, id_host_tab, offset=18.5, width=.07, colnames=TRUE)+scale_fill_manual(breaks=c("Human","Bird","Mosquito","Horse"), values=c("#882255","#DDCC77","darkgrey","darkgreen"), name="Host")
```
...
allTrees = scan(file="wnv_L1_mediterranean_clock_subset_trimmed.trees", what="", sep="\n", quiet=TRUE)
burnIn = 1000
...
mcc_tre = readAnnotatedNexus("wnv_L1_mediterranean_clock_subset_trimmed.tre")
...
world_spatial <- as_Spatial(world)
world_raster <- raster(crs = crs(world), vals = 0, resolution = c(0.05, 0.05), ext = extent(c(-13, 23, 33, 48))) %>% rasterize(world, .)
